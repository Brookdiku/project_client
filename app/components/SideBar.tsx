...
export default function SideBar({ isMenuToggled, handleToggle }) {
  //className={`fixed left-0 top-0  bg-blue-600 h-full p-4  sidebar-menu transition-transform shadow-md shadow-black/5`}
  return (
    <>
      <div
        className={`w-full p-4 flex flex-col h-full ${
          isMenuToggled ? "items-center" : "items-start"
        }`}
      >
        ...
        <div className="flex flex-col gap-2 mt-5 w-full items-center">
          <Button
            isIconOnly={!isMenuToggled ? false : true}
            variant="flat"
            className={`${
              !isMenuToggled ? "w-full justify-start" : "justify-center"
            } flex items-center rounded-md`}
          >
            <a href="/admin" className="text-left">
              {isMenuToggled ? (
                <Tooltip placement="right" content="Dashboard" className="p-4">
          ...
          >
            <a href="/admin/products" className="text-left">
              ...
              {!isMenuToggled ? (
                <span className="text-sm ml-3 ">Products</span>
              ) : (
                <></>
              )}
            </a>
          ...
          >
            <a href="/admin/categories" className="text-left">
              ...
              {!isMenuToggled ? (
                <span className="text-sm ml-3 ">category</span>
              ) : (
                <></>
              )}
            </a>
