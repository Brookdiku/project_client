...
const SignUpForm: React.FC<SignUpFormProps> = () => {
  ...
  // Event handler for response messages
  const ResponseFunctionBuilder = (message: string, flag: boolean, type: string) => {
    setResponseMessage(message);
    setType(type);
    setFlag(flag);
  ...
  return (
    <div className='min-h-screen w-full bg-background flex items-center justify-center'>
      <Toast message={responseMessage} flag={flag} type={type} />
      <section className='w-full h-full grid gap-2 md:grid-cols-1 lg:grid-cols-2 md:w-10/12 lg:w-8/12'>
        <div className='hidden lg:block bg-blue-500 rounded-l-md'></div>
        <div className='w-full h-full p-4 md:p-8 lg:rounded-r-md flex flex-col gap-3'>
          <p className='text-2xl text-center font-bold text-blue-500'>ACMERA</p>
